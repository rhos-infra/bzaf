...
def main():
    ...
    if bzids:
        ...
        for bz in bzids:
            ...
            try:
                ...
                if bug.status != current_status:
                    logger.debug('BZ #{i} status does not '
                                 'match {s}'.format(i=bz,
                                                    s=current_status))
                else:
                    logger.debug('BZ #{} is valid'.format(bz))
                    valid_bugs.append(bug)
            except Exception as e:
                logger.debug('Bug #{bi} is not valid, '
                             'reason: {err}'.format(bi=bz,
                                                    err=e))

        if not valid_bugs:
            raise exceptions.bzafNoValidBugs()
        logger.info('Valid bugs:')
        ...
        for valid_bug in valid_bugs:
            ...
            for comment in reversed(comments):
                if private_comment:
                    if not comment['is_private']:
                        logger.error("Can't parse from non private comment #{}"
                                     .format(comment['count']))
                        continue
                text = comment['text']
                # Attempt to validate comments with YAML
                print('Iterrating over comment #{}'.format(comment['count']))
            ...
            if bzaf_found:
                print("Located valid bzaf spec in comment")
                ...
                if bzaf_execution:
                    ...
                    try:
                        ...
                        logger.debug('Updated bug #{}'.format(valid_bug.id))
                    except Exception as e:
        ...
        summary.prepare_summary(bzids, valid_bugs, valid_bugs)
